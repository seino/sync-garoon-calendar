...
function parseGaroonTargets(targetsStr: string): GaroonTarget[] {
  ...
  for (const part of parts) {
    ...
    if (type && id && (type === 'user' || type === 'organization')) {
      ...
      log.warn('無効なターゲット形式をスキップしました', { value: part });
    }
  }
...
/**
 * 設定ファイル（JSON）で設定を上書きする（後方互換性）
 */
function applyFileOverrides(config: AppConfig, configPath: string): void {
  try {
    ...
    log.warn('設定ファイルの読み込みに失敗しましたが、デフォルト値と環境変数で続行します', { error });
  }
}
...
export function loadConfig(configPath?: string): AppConfig {
  try {
    ...
    if (configPath) {
      applyFileOverrides(config, configPath);
    }
    return config;
  } catch (error) {
